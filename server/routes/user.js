...
router.post('/api/register', async (req, res) => {
    try {
        ...
        const [result] = await db.query('INSERT INTO user (username, password, role) VALUES (?, ?, ?)', [username, hashpwd, 'normal'])
...
router.post('/api/login', async (req, res) => {
    try {
        ...
        const [rows] = await db.query('SELECT id, username, password, role FROM user WHERE username = ?', [username])
        if (rows.length === 0) {
...
// router.post('/api/login/0', async (req,res) => {
//    try{
//      const {username, password} = req.body
//     if(!username || !password){
//         return res.json({
...
//     const [rows] = await db.query('SELECT id, username, password, role FROM user WHERE username = ? and role = ?;', [username, 'admin'])
//     if(rows.length === 0){
...
router.post('/upload', tokenAuth, upload.single('avatar'), async (req,res) =>{
   try{
     ...
     if(req.auth.role){
        const [result] = await db.query('UPDATE user SET avatar=? WHERE id=?', [avatarPath, req.auth.id])
        ...
        const [result] = await db.query('UPDATE student SET avatar=? WHERE id=?', [avatarPath, req.auth.id])
        if(result.affectedRows === 0){
            return res.status(404).json({
                code:404,
                message:'没找到这个学生'
...
router.post('/admin/modify', tokenAuth, async (req,res) =>{
    try{
        ...
        const [rows] = await db.query('SELECT * FROM user WHERE id = ? AND username = ?', [req.auth.id, username])
            if(rows.length === 0){
               return res.status(401).json({
                 code:401,
                message:'没找到该用户'
...
router.get('/admin/user', tokenAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const [rows] = await db.query('SELECT id, username, role, avatar FROM user LIMIT ? OFFSET ?', [limit, offset])
...
router.post('/admin/user', tokenAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const hashpwd = await bcrypt.hash(password, saltRounds)
        const [result] = await db.query('INSERT INTO user (username, password, role) VALUES (?, ?, ?)', [username, hashpwd, userRole])
        if (result.affectedRows === 1) {
            return res.status(200).json({
                code: 200,
                message: '用户添加成功',
...
router.put('/admin/user/:id', tokenAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const [dup] = await db.query('SELECT id FROM user WHERE id <> ? AND username = ?', [id, username])
        if (dup.length > 0) {
            return res.status(400).json({
                code: 400,
                message: '用户名已存在'
